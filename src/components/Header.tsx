      ...
      <nav className="container mx-auto px-4 py-4">
        <div className="flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center space-x-8">
            {navItems.map((item) => (
              <a
                key={item.name}
                href={item.href}
                className="text-white font-medium hover:text-urban-neon transition-colors duration-300 relative group"
              >
                {item.name}
          ...
          <div className="flex items-center space-x-4">
            ...
            {/* Mobile Menu Button */}
            <button
              className="md:hidden text-white hover:text-urban-neon transition-colors duration-300"
            ...
            <div className="px-4 py-6 space-y-4">
              {navItems.map((item) => (
                <a
                  key={item.name}
                  href={item.href}
                  className="block text-white font-medium hover:text-urban-neon transition-colors duration-300"
